...
void BSpline::setControlPoints(vector<Point2D*> *p)
{
	ctrlPts = p;
}
...
Point2D *BSpline::effSum(int d, float u, vector<Point2D*> *geoPts, vector<Point2D*> *convexPts)
{
	...
	for (int r = k; r >= 2; r--)		// the columns of the table
	{
		...
		for (int s = 0; s <= r - 2; s++)	// the elements of the column
		{
			...
			{
				geoPts->push_back(new Point2D(c[s]));	// add the non-final point to the geometric list
				if (s > 0 && s < r - 2)
					geoPts->push_back(new Point2D(c[s]));	// add the same point twice if its shared between two lines
			}
		}
	}
